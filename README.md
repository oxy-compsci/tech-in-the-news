...
* [Generating new molecules with graph grammar](https://news.mit.edu/2022/generating-new-molecules-with-graph-grammar-0401)

  Researchers at MIT and IBM have worked on making a data-efficient machine learning model for discovering new chemical compounds; current methods require extensive amounts of data to build new molecules. To achieve this, they utilized "graph grammar", or a system of structures for word ordering, but instead, they contain rules about the creation or building of molecules such as polymers and monomers. To go further, the molecular structure can be thought of as a graph, with a string of atoms (nodes) joined together by chemical bonds (edges); the process of generating molecules can be repeated and therefore new rules can be built as well. Not only does this work forward and backwards, but like a tree (sort of), you can try all sorts of different combinations by backtracking and trying something new. By starting with a few predictable rules, it's possible to be very accurate with determining how the new, created molecules will act, react, and look. Performance-wise, the model performs just as well, while requiring way less data: "[the algorithm only requires] 0.15 percent of the original dataset to achieve very similar results compared to state-of-the-art approaches that train on tens of thousands of samples." As this is graph grammar, but requires less data, the team calls it "data-efficient graph grammar" (DEG) and plan to utilize and present it in the future. As this project has been a success (so far), the next steps are to build up this 'graph grammar' in the form of new rules; this will allow the graphic representation to be used across many domains.
